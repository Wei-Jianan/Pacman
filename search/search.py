...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while True:
        ...
        successors = problem.getSuccessors(current_place)
        for temp in successors:
            frontier.push((temp[0], current_path + [temp[1]], current_cost + temp[2]), current_cost + temp[2] + heuristic(temp[0], problem))#f(temp[2],problem))
        while (True):
            if (frontier.isEmpty()):
                return None
            temp = frontier.pop()
            if temp[0] not in explored:
                break
        current_node = (temp[0], temp[1], temp[2])
        explored.append(temp[0])
    return current_node[1]
