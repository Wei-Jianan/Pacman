...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        for i in range(iterations):
            for state in mdp.getStates():
                ...
                # to deal with terminal state stituation
                if len(q_values) == 0:
                    q_values.append(0)
                next_values[state] = max(q_values)
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for next_state, probability in self.mdp.getTransitionStatesAndProbs(state, action):
            ...
            q_value += probability * (self.mdp.getReward(state, action, next_state) + self.discount * self.getValue(next_state))
        return q_value
